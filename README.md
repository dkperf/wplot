...
Make sure to Try:  'wplot -h > wplot.hlp'  then 'wplot wplot.hlp'.    There are also a few .dpt command file examples in the examples folder.  Just download the examples folder contents and then type "wplot xxx.dpt" to see that particular plot example.
...
```
   ...
   Usage: wplot <cmdOpts>  <command file>  or  <raw data file>
  
   <cmdOpts>  
      -                :: show dialog box with command line options.
      ...
      -h               :: help (must send output (>) to file).
   ...
   Mostly used when doing direct read of data file; bypassing cmd file.
   ...
   Cmd file:
      ...
      xdate         - x axis data type (expects mm-/dd-/yy(yy) 
                       or mm-/dd-/yy(yy)_HH:MM:SS in data file). '_' can
                       be a space if space not in seperators. ie. 'seps c'
      ...
      xmax    <val> - set a max x value for initial display.
      ymax    <val> - set a max y value for initial display.
      xmin    <val> - set a min x value for initial display.
      ymin    <val> - set a min y value for initial display.
      ...
      usage       - output usage text to stdout and exit. Same as -h.
      ...
      plot <filename> <options>  - one 'plot' command for each object.
         <options>  in any order :
             ...
             hline vline           ( object type )
             bars                  ( object type; sizecol adjusts base, allows stacking )
             hspike vspike         ( object type; use sizecol to set length )
             labels                ( object type; needs labelcol; uses colorcol, sizeccol )
                                     quotes not recognized in col data. Use seps for txt w/spaces
             ...
             frame hatch hatch2-5  ( modify polygon, bars)
             small medium  large   ( apply to dots )
             solid dash dotdash    ( apply to line; only thin lines)
             %<percentage>         ( apply to bars; bar width is % of min gap between all x )
                                     values. If % number is negative then bar width will be 
                                     in units of x axis data type. ie. 'xdate' is seconds...
             ...
             x<mod>:y<mod>         ( col nums for x and y data. 1 based;no spaces) def 1:2
                                     <mod> can be *i, /i, +i or -i where i is a float.
                                       allows each x or y item to be modified by i.
                                     If x col is set to 0 then internal counter is used
                                       for x.  Allows only y data in file.
                                     If y col is set to 0 then all y data will be 0.
             opt                   ( x or y col data in file is optional. No Warn/Err if missing)
      ...
      If <filename> starts with a ':<tag>' then data lines are in this cmd file
    ...
    Color Info:
        # from 1-32 or xRRGGBB for RGB color definition;  _HHSSLL for HSL color definition.
        ...
        You can press 'Add to Custom Colors' to add multiple colors, then press OK.

    Polygon and other objects:  A blank line in the data file will complete a polygon or 
    segment and start another. ie. One 'plot' obj can have multiple polygons/objects in it.

   Example cmd file:    ( more examples below )
   ...
   User interface: 
     ...
     Buttons:  Mouse over button and read bottom status line/tooltip for definition.
  
     Mouse:
        left button      -  pan screen
        Ctrl left button -  show crossHair at cursor.
                            status line updated with X,Y Position and
                            delta X,Y values for measurement.
        Shift left button-  zoom in with selection rectangle
        alt   left button-  full view.
        ...
        Ctrl             -  cursor X:Y tooltip and status bar shows obj name.
...
plot :barw   1+10:2-10  t barTxt  bars  bt:c6:s150  %15  x408080 hatch3
